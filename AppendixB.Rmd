...
T_series <- read_csv("ReadFiles/ExampleInputFile.csv")

T_series.t <- T_series %>% 
  mutate(r_DT =    mdy_hm(TimeCode)) %>% 
  filter(!is.na(Ion1))
...
1
cvi.2 <- t(data.frame(cvi(cluster_dtw_h[[2]], type = "valid")))
cvi(cluster_dtw_h[[3]], type = "valid")
